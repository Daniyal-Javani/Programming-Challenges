var i, count = 0;
numerator = new BigInt("3");
denumerator = new BigInt("2");
for (i = 1; i < 1000; ++i) {
	// numerator += denumerator;
	numerator = bigint_plus(numerator, denumerator);
	temp = numerator.toString();
	numerator = new BigInt(denumerator.toString());;
	denumerator = new BigInt(temp.toString());
	numerator = bigint_plus(numerator, denumerator);
	if (numerator.toString().length > denumerator.toString().length) {
		++count;
	}
}
console.log(count);
